...
export class AIService {
  ...
  async generateHint(
    ...
    try {
      ...
      return {
        ...
        confidence: 0.5,
        suggestedApproach:
          'Start by identifying the input parameters and expected output format.',
  ...
  async reviewCode(code: string, _language: string) {
    try {
      const result = await this.claudeService.analyzeCode(code, _language);
  ...
  async generateExercise(topic: string, difficulty: string, _language: string) {
  ...
  async getPersonalizedRecommendations(_userId: string) {
