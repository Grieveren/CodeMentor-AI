...
export const specificationTheme: editor.IStandaloneThemeData = {
  ...
  rules: [
    ...
    { token: 'requirement-reference', foreground: '800000', fontStyle: 'underline' },
...
export const specificationTokenProvider: languages.IMonarchLanguage = {
  tokenizer: {
    root: [
      ...
      [/^\d+\.\s+(WHEN|IF|WHILE|WHERE|AS LONG AS).*THEN.*SHALL.*/, 'ears-format'],
  ...
  provideCompletionItems: (model, position) => {
    ...
    const suggestions: languages.CompletionItem[] = [
      // User story templates
      {
        ...
        documentation: 'Insert a user story template with acceptance criteria',
        range,
      },
...
export const specificationHoverProvider: languages.HoverProvider = {
  provideHover: (model, position) => {
    ...
    if (line.includes('WHEN') && line.includes('THEN') && line.includes('SHALL')) {
      return {
        range: new Range(position.lineNumber, 1, position.lineNumber, line.length + 1),
        contents: [
          { value: '**EARS Format Requirement**' },
          { value: 'Event-driven requirement using Easy Approach to Requirements Syntax (EARS)' },
          { value: 'Format: WHEN [event] THEN [system] SHALL [response]' },
        ],
      };
    }
    
    if (line.includes('IF') && line.includes('THEN') && line.includes('SHALL')) {
      return {
        range: new Range(position.lineNumber, 1, position.lineNumber, line.length + 1),
        contents: [
          { value: '**EARS Format Requirement**' },
          { value: 'Conditional requirement using Easy Approach to Requirements Syntax (EARS)' },
    ...
    if (line.includes('As a') && line.includes('I want') && line.includes('so that')) {
      return {
        ...
        contents: [
          { value: '**User Story**' },
          { value: 'Standard user story format for capturing requirements from user perspective' },
...
export const specificationDiagnosticsProvider = (model: editor.ITextModel): editor.IMarkerData[] => {
  ...
  lines.forEach((line, lineIndex) => {
    ...
    if (line.includes('As a') && (!line.includes('I want') || !line.includes('so that'))) {
      markers.push({
        ...
        message: 'Incomplete user story format. Expected: As a [role], I want [feature], so that [benefit]',
        code: 'incomplete-user-story',
      });
    }
    
    // Check for malformed EARS requirements
    if ((line.includes('WHEN') || line.includes('IF')) && !line.includes('SHALL')) {
      markers.push({
    ...
    if (line.match(/^-\s+\[\s*\]\s+\d+/) && !line.includes('_Requirements:') && !content.includes(`_Requirements: ${line.match(/\d+(\.\d+)*/)?.[0]}_`)) {
      markers.push({
...
export const initializeSpecificationEditor = () => {
  ...
  languages.setLanguageConfiguration('specification', specificationLanguageConfig);
  
  // Set tokenization provider
  languages.setMonarchTokensProvider('specification', specificationTokenProvider);
  
  // Register completion provider
  languages.registerCompletionItemProvider('specification', specificationCompletionProvider);
...
export const getSpecificationEditorOptions = (readOnly = false): editor.IStandaloneEditorConstructionOptions => ({
...
export const validateSpecificationDocument = (content: string, type: 'requirements' | 'design' | 'tasks') => {
  const lines = content.split('\n');
  const issues: { line: number; message: string; severity: 'error' | 'warning' | 'info' }[] = [];
  ...
  switch (type) {
    case 'requirements': {
      ...
      if (!hasUserStories) {
        issues.push({ line: 1, message: 'Requirements document should contain user stories', severity: 'warning' });
      }
      if (!hasAcceptanceCriteria) {
        issues.push({ line: 1, message: 'Requirements document should contain acceptance criteria', severity: 'warning' });
      }
      break;
    }
    ...
    case 'design': {
      ...
      if (!hasArchitecture) {
        issues.push({ line: 1, message: 'Design document should include architecture section', severity: 'warning' });
      }
      if (!hasComponents) {
        issues.push({ line: 1, message: 'Design document should include components section', severity: 'info' });
      }
      if (!hasDataModels) {
        issues.push({ line: 1, message: 'Design document should include data models section', severity: 'info' });
      }
      break;
    }
    ...
    case 'tasks': {
      ...
      lines.forEach((line, index) => {
        if (line.match(/^-\s+\[\s*\]/)) {
          ...
          if (!line.includes('_Requirements:') && !content.includes(`_Requirements: ${line.match(/\d+(\.\d+)*/)?.[0]}_`)) {
            issues.push({
              line: index + 1,
      ...
      if (!hasTaskItems) {
        issues.push({ line: 1, message: 'Task document should contain task items', severity: 'warning' });
      }
      break;
    }
  }
