...
interface LessonsStore {
  ...
  fetchTracks: (params?: { category?: string; difficulty?: string; search?: string }) => Promise<void>;
  ...
  // Actions - Navigation
  getNextLesson: (currentLessonId: string, trackId?: string) => Promise<Lesson | null>;
  getPreviousLesson: (currentLessonId: string, trackId?: string) => Promise<Lesson | null>;
    ...
    (set, get) => ({
      ...
      fetchLessons: async (params?: LessonQueryParams) => {
        ...
        try {
          ...
          const response: PaginatedResponse<Lesson> = await lessonsService.getLessons(queryParams);
      ...
      fetchTrackLessons: async (trackId: string) => {
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Failed to fetch track lessons';
          setError('lessons', errorMessage);
      ...
      fetchFeaturedLessons: async (limit = 6) => {
        ...
        try {
          const featuredLessons = await lessonsService.getFeaturedLessons(limit);
          set({ featuredLessons });
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Failed to fetch featured lessons';
      ...
      fetchRecommendedLessons: async (limit = 10) => {
        ...
        try {
          const recommendedLessons = await lessonsService.getRecommendedLessons(limit);
          set({ recommendedLessons });
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Failed to fetch recommended lessons';
      ...
      fetchRecentLessons: async (limit = 10) => {
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Failed to fetch recent lessons';
      ...
      fetchPopularLessons: async (limit = 10) => {
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Failed to fetch popular lessons';
      ...
      getPreviousLesson: async (currentLessonId: string, trackId?: string) => {
        try {
          return await lessonsService.getPreviousLesson(currentLessonId, trackId);
        } catch (error) {
