...
export const useSpecificationProject = (projectId?: string) => {
  ...
  const createProject = useCallback(
    async (projectData: CreateProjectData): Promise<SpecificationProject> => {
      setLocalError(null);
      try {
  ...
  const updateProject = useCallback(
    async (updates: ProjectUpdates): Promise<void> => {
  ...
  const setCurrentProject = useCallback(
    (project: SpecificationProject | null) => {
      setLocalError(null);
  ...
  const updateProjectPhase = useCallback(
    async (phase: SpecificationPhase): Promise<void> => {
  ...
  const canCompleteProject = useCallback((): boolean => {
    const { phaseValidation } = store;
    const requiredPhases: SpecificationPhase[] = [
      'requirements',
      'design',
      'tasks',
    ];
...
export const useSpecificationProjects = () => {
  ...
  const getProjectsByStatus = useCallback(
    (status: ProjectStatus): SpecificationProject[] => {
  ...
  const getProjectsByPhase = useCallback(
    (phase: SpecificationPhase): SpecificationProject[] => {
  ...
  const getRecentProjects = useCallback(
    (limit: number = 5): SpecificationProject[] => {
